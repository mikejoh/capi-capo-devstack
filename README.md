...
The steps above get you a steady-state, kube-proxy-free cluster with Cilium.
A few directions to take it further, kept out of this README so it stays a
straight read-through:

- **[Dynamic OpenStack credentials with OpenBao](docs/dynamic-openstack-credentials.md)**
  — replace the hand-filled `clouds.yaml` from step 9 with a short-lived
  credential minted on demand, so nothing static and non-expiring survives
  past one root identity.
- **[Talos instead of kubeadm](docs/talos-instead-of-kubeadm.md)** — same
  OpenStack plumbing, [Talos Linux](https://www.talos.dev/) as the node OS
  instead of kubeadm.
